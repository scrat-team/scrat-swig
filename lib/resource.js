...
var DEFAULT_COMBO_SPLIT_LENGTH = 1920; //1024 + 512 + 256 + 128
 ...
 * @param collect {Array}
 */
Resource.prototype.genComboURI = function(collect, url_length) {
  var url = this.getComboPattern();
  var len = typeof(url_length) != 'undefined' ? url_length : Number.POSITIVE_INFINITY;// 正无穷
  var output = [];
  var combo = '';
  var temp;
 ...
 * @returns {number}
 */
Resource.prototype.getComboMaxLen = function() {
  return options.comboMaxLen > -1 ? options.comboMaxLen : DEFAULT_COMBO_SPLIT_LENGTH;
};
...
Resource.prototype.renderJs = function() {
  ...
  if (this._collect.js && this._collect.js.length) {
    ...
    var right = '"></script>\n';
    if (options.combo) {
      var pack = this.genComboURI(this._collect.js, this.getComboMaxLen());
...
Resource.prototype.genLinkTag = function(collect, defer) {
  ...
  if (collect && collect.length) {
    ...
    if (options.combo) {
      var pack = this.genComboURI(collect, this.getComboMaxLen());
      pack.forEach(function(res) {
        html += left + res + right;
      });
    } else {
