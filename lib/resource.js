...
Resource.setRoot = function(path) {
  var rootPath = path.replace(/\/((views|view)?\/?)?$/, ''); // historical problems
  if (rootPath) {
    root = rootPath;
  }
};
...
Resource.prototype.includeCSS = function(file) {
  var hasExt = /\.css$/.test(file);
  var normalizeFile = this.normalize(hasExt ? file : file + '.css', '.css');
  var res = this.getResById(normalizeFile);

  if (res) {
    var collect = this._useATF ? this._includeCssATF : this._includeCss;
...
Resource.prototype.renderJs = function() {
  ...
  var code = 'pagelet&&pagelet.init&&pagelet.init(' + args.join(',') + ');';
...
Resource.prototype.renderCss = function() {
  var collect = (this._collect.css || []).concat(this._includeCss);
...
Resource.prototype.renderATFCss = function() {
  var html = '';
  var _collect = this._collect.css;
  var collect = _collect && _collect.length ?
  ...
  if (collect && collect.length) {
    ...
    for (var i = 0; i < collect.length;) fn(collect[i++]);
...
Resource.prototype.renderBTFCss = function() {
  var _collect = this._collectATF.css || [];
  var collect = _collect.concat(this._includeCssATF);
  var html = this.genLinkTag(collect, true);
