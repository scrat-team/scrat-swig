...
Resource.prototype.renderCss = function (isATF) {
    var html = '';
    var _collect = isATF ? this._collectATF : this._collect;
    if (_collect.css && _collect.css.length) {
        var left = '<link rel="stylesheet" href="';
        var right = '">\n';
        if (options.combo) {
            html += left + this.genComboURI(_collect.css) + right;
        } else {
            html += join(_collect.css, left, right);
        }
    }
    return html;
};
