 ...
 * @returns {string}
 */
Resource.prototype.renderCss = function (isATF) {
    var html = '';
    if(isATF){
        if(this._collectATF.css && this._collectATF.css.length){
            if(options.combo){
                var uri = '"' + this.genComboURI(this._collectATF.css) + '"';
            } else {
                uri = '"' + this._collectATF.css.join('","') + '"';
            }
            html = '<script>' +
                   '(function(a,b,e){' +
                       'a.forEach(function(c){' +
                           ...
                           'e.appendChild(d)' +
                       '})' +
                   '})([' + uri + '],document,document.head||document.getElementsByTagName("head")[0])' +
                   '</script>';
        }
    } else {
        if (this._collect.css && this._collect.css.length) {
            ...
            var right = '">\n';
            if (options.combo) {
                html += left + this.genComboURI(this._collect.css) + right;
            } else {
...
Resource.prototype.renderATFCss = function () {
    var html = '';
    if(this._collect.css && this._collect.css.length){
        this._collect.css.forEach(options.combo ? function(uri){
            var content = '';
...
Resource.prototype.render = function (out) {
    if (this._usePagelet) {
        ...
        if(this._useATF){
            ...
            if (p > -1) {
                out = out.substring(0, p) + this.renderATFCss() + out.substring(p + this.CSS_HOOK.length);
            }
            p = out.indexOf(this.ATF_HOOK);
            if(p > -1){
                out = out.substring(0, p) + this.renderCss(true) + out.substring(p + this.ATF_HOOK.length);
            }
        } else {
            p = out.indexOf(this.CSS_HOOK);
            if (p > -1) {
